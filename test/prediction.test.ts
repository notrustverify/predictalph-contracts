...
describe("unit tests", () => {
  ...
  async function checkRoundState(epoch: bigint, amount: bigint) {
    const roundContractId = getSubPredictContractId(
  ...
  async function getRoundBidder(address: Address, epoch: bigint) {
    const bidderContractId = getSubPredictContractId(
  ...
  test("create round, boost it", async () => {
    ...
    const predictionGame = getPredictObject(0n);
    ...
    await boostRound(operator, predictionGame, 0n, 10n * ONE_ALPH);
    ...
    const roundState = await getRoundState(predictionGame.contractId, 0n);
    ...
    const roundStateWithdraw = await getRoundState(
      predictionGame.contractId,
      0n
    );
  ...
  test("create round, 5 bidders, none won, withdraw", async () => {
    ...
    const roundStateWithdraw = await getRoundState(predictionGame.contractId,0n);
  ...
  test("create round, end it, cannot destroy", async () => {
    ...
    await expectAssertionError(
      ...
      addressFromContractId(
         getSubContractIdByContractId(predictionGame.contractId,getEpochPath(predictionState.fields.epoch - 1n))
      ),
      101
    );
  });
