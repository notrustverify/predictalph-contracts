...
async function startRound(privKey: string, contractName: string) {
  ...
  try {
    ...
    console.log(`Start round ${predictionStates.fields.epoch} ${txStart.txId}`);
...
async function endRound(
  ...
  let isMultipleChoiceContract;
  let isChoiceContract = false;
  ...
  const moveRound = async () => {
    if (contractName.split(":")[0].toLowerCase() == "predictchoice") {
    ...
    if (roundExists) {
      roundState = await getRoundContractState(
        ...
        wallet.group,
        isChoiceContract,
        isMultipleChoiceContract
      );

      endTimestamp = Number(roundState.fields.bidEndTimestamp);
      if (roundState.fields.endBeforeEnd) {
        console.log("Round can be ended before");
      }
      try {
        if (Date.now() >= endTimestamp || roundState.fields.endBeforeEnd) {
          console.log(
            `locking for ${TIME_TO_WAIT_NEW_ROUND / (60 * 1000)} minutes`
          );
          ...
          } else if (isMultipleChoiceContract) {
            const sideWonBigInt = BigInt(sideWon)

            txStart = await EndMultipleChoice.execute(wallet, {
              initialFields: {
                predict: predictContractId,
                sideWon: sideWonBigInt,
                immediatelyStart: immediatelyStart,
              },
              attoAlphAmount: ONE_ALPH,
            });
...
const sideWon = process.argv.slice(2)[3];
