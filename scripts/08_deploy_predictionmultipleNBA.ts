...
const deployPredictionMultipleChoice: DeployFunction<Settings> = async (
  ...
  const END_TIMESTAMP = 1717633800
  const settings = network.settings
  const title = "Dallas Mavericks vs Boston Celtics"
  ...
  const predict = await deployer.deployContract(PredictMultipleChoice, {
    initialFields: {
       ...
       repeatEvery: BigInt(END_TIMESTAMP * 1000),
       claimedByAnyoneDelay: BigInt((ONE_WEEK_SEC + END_TIMESTAMP) * 1000),
       title: binToHex(new TextEncoder().encode(title)),
       playerCounter: 0n,
       endBeforeEnd: true
    },

  },"PredictMultipleChoiceNBADallasBoston")
